...
const ClassificaGenerale_Board: React.FC = () => {
  const { scores, bonuses, resetAll } = useScores();
        ...
        {teamConfigs.map((team, i) => (
          <div key={i} className="flex flex-col items-center w-full max-w-sm">
            ...
            <div className="flex-1 w-full flex flex-col items-center justify-end mb-10 relative">
              <div className="text-5xl font-black mb-4 animate-pulse">
                {scores[i].toLocaleString()} <span className="text-xl">PT</span>
              </div>
              
              <div className="w-full bg-white/5 rounded-t-3xl border-x-2 border-t-2 border-white/10 relative overflow-hidden group" style={{ height: '400px' }}>
                <div 
                  className={`absolute bottom-0 left-0 w-full ${team.color} transition-all duration-1000 ease-out shadow-[0_0_40px_rgba(255,255,255,0.1)]`}
            ...
            <div className="w-full bg-white/5 p-6 rounded-3xl border-2 border-white/10 flex justify-center gap-6">
              {[0, 1, 2].map(bonusIdx => (
                ...
                >
                  <img 
                    src={`/Icone/${bonusIdx === 0 ? 'Primo' : bonusIdx === 1 ? 'Secondo' : 'Terzo'} indizio.svg`} 
                    alt="Bonus"
                    className="w-12 h-12"
                    onError={(e) => {
